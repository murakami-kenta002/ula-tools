...
Please add the application name, which is the same as defined in initial_vscreen.json or dwm_initial_layout.json, with "-i" option.
...
The sample virtual-screen-def.json is located in the "$GOPATH/src/ula-tools/example/rvgpu/virtual-screen-def.json". 
...
The sample initial_vscreen.json is located in the "$GOPATH/src/ula-tools/example/rvgpu/initial_vscreen.json". 

After creating initial_vscreen.json, please run the command below.
```
cat <path to initial-vscreen.json> | ula-distrib-com <path to virtual-screen-def.json>
```
